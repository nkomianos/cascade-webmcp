...
export default function CascadeApp() {
  ...
  const [newPlan, setNewPlan] = useState(launchPlan);
  const [newPlanStarter, setNewPlanStarter] = useState<'launch' | 'community' | 'blank'>('launch');
  ...
  const openNewPlan = () => {
    setNewPlanStarter('launch');
    setNewPlan(launchPlan);
    setModal('new-plan');
  };
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <button className="decision-title plain-button" onClick={openNewPlan}><span>Decision</span><strong>{workspace.title}</strong></button>
        <div className="top-actions"><button className="ghost-button" onClick={openNewPlan}>New plan</button><span className={`tool-status ${toolStatus}`}><i />{toolStatus === 'ready' ? '11 agent tools ready' : toolStatus === 'checking' ? 'Checking tools' : 'Preview mode'}</span><button className="outline-button" onClick={exportWorkspace}>Export</button></div>
      </header>
